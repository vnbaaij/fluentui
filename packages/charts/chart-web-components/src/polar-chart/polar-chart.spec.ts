...
const data: PolarChartSeries[] = [
  { legend: 'Series A', data: [{ x: 'Speed', y: 8 }, { x: 'Power', y: 5 }, { x: 'Agility', y: 7 }] },
  { legend: 'Series B', data: [{ x: 'Speed', y: 4 }, { x: 'Power', y: 9 }, { x: 'Agility', y: 3 }] },
];
...
test.describe('PolarChart', () => {
  ...
  test('Should re-render on data change', async ({ page }) => {
    const element = page.locator('fluent-polar-chart');
    const nextData: PolarChartSeries[] = [{ legend: 'Series C', data: [{ x: 'Speed', y: 6 }, { x: 'Power', y: 4 }, { x: 'Agility', y: 8 }] }];
